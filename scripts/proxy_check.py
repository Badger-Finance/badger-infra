from brownie import network, interface, web3, BadgerRegistry
from helpers.constants import REGISTRY, AddressZero
...
def check_proxy_admin(proxy, proxyAdmin, key):
    ...
    address = web3.eth.getStorageAt(proxy, ADMIN_SLOT).hex()

    # Check differnt possible scenarios
    if address == "0x":
        console.print(key, ":[yellow] admin not found on slot (Not a proxy?)[/yellow]")
...
def check_proxy_admin_owners(proxyAdminOwners, registry):
    ...
    for adminOwnerPair in proxyAdminOwners:
        ...
        address = web3.eth.getStorageAt(proxyAdmin, 0).hex()
